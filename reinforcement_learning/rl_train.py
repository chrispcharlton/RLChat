...
def train(load_dir='data\\save\\cb_model\\cornell movie-dialogs corpus\\2-2_500', save_dir="data\\rl_models\\DQNseq2seq", num_episodes=50, env=None):
    ...
    for i_episode in range(1, num_episodes+1):
        ...
        while not done:
            length += 1
            action, prob = searcher(state)
            reward, next_state, done = env.step(action)
            reward = torch.tensor([reward], device=device)
            memory.push(state, action, next_state, reward, done, torch.mean(prob))
            state = next_state
        print("Episode {} completed, lasted {} turns.".format(i_episode, env.n_turns))
        loss = optimize_batch_q(policy, policy_optimizer, searcher, memory, encoder_optimizer, decoder_optimizer)
            # if done:
